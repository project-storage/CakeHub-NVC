    ...
    async getStatistics() {
        const [totalOrders, revenueAgg, pendingPaymentAgg, cakesSold, topSellingCakes] = await Promise.all([
            ...
            this.prisma.order.aggregate({
                ...
                where: { status: { in: [client_1.OrderStatus.PAID, client_1.OrderStatus.DELIVERED] }, deletedAt: null },
            }),
            this.prisma.order.aggregate({
            ...
            this.prisma.orderDetail.aggregate({
                _sum: { quantity: true },
                where: { order: { deletedAt: null, status: { not: client_1.OrderStatus.CANCELLED } } },
            }),
