...
export class StudentsService {
  ...
  async findAll(user: UserPayload, page: number = 1, limit: number = 10, search?: string) {
    ...
    const [data, total] = await Promise.all([
      this.prisma.student.findMany({ where, skip, take: limit, include: { group: true } }),
  ...
  async findOne(id: number, user: UserPayload) {
    ...
    if (user.role === Role.ADVISOR) {
        if (!record.groupId) throw new ForbiddenException("You do not have access to this student");
        const group = await this.prisma.group.findFirst({
            where: { id: record.groupId, advisorId: user.id }
        });
        if (!group) throw new ForbiddenException("You do not have access to this student");
    }
