...
export class ReportService {
  ...
  async getOrderSummary(res?: Response, exportType?: string) {
    ...
    if (exportType === 'excel' && res) {
      ...
      orders.forEach(o => {
        sheet.addRow([o.id, o.student?.fullName || 'N/A', o.totalPrice, o.status, o.createdAt]);
      });
    ...
    if (exportType === 'pdf' && res) {
      ...
      orders.forEach(o => {
        doc.fontSize(12).text(`Order ID: ${o.id} | Student: ${o.student?.fullName || 'N/A'} | Total: ${o.totalPrice} | Status: ${o.status}`);
        doc.moveDown(0.5);
      });
  ...
  async getRevenueSummary(res?: Response, exportType?: string) {
    ...
    if (exportType === 'pdf' && res) {
      ...
      revenue.forEach(r => {
        doc.fontSize(12).text(`Payment ID: ${r.id} | Order ID: ${r.orderId} | Amount: ${r.amount} | Type: ${r.type}`);
        doc.moveDown(0.5);
      });
